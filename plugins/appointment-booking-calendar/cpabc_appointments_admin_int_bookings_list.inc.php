...
<table class="wp-list-table widefat fixed pages" cellspacing="0">
	...
	<tr>
	  ...
	  <?php } ?>
      <?php if ($orderby == 'id DESC' || $orderby == 'id ASC') { ?>
      <th style="padding-left:7px;font-weight:bold;">Submission Time</th>
      <?php } ?>
	  ...
	  <th style="padding-left:7px;font-weight:bold;">FollowUp</th>
	...
	<tbody id="the-list">
	 <?php for ($i=($current_page-1)*$records_per_page; $i<$current_page*$records_per_page; $i++) if (isset($events[$i])) { ?>
	  <tr class='<?php if (!($i%2)) { ?>alternate <?php } ?>author-self status-draft format-default iedit' valign="top">
	    ...
	    <?php } ?>
        <?php if ($orderby == 'id DESC' || $orderby == 'id ASC') { 
           $eventstime = $wpdb->get_results( "SELECT time FROM ".$wpdb->prefix."cpabc_appointments WHERE id=".intval($events[$i]->reference) );
        ?>
        <td <?php if ($events[$i]->is_cancelled == '1') { ?>style="color:#faabbb;"<?php } ?>><?php echo $eventstime[0]->time; ?></td>
        <?php } ?>
