...
<script type="text/javascript">
 ...
 var cpabc_max_slots_text = '<?php echo str_replace("'","\'",__('Please select a maximum of %1 time-slots. Currently selected: %2 time-slots.','cpabc')); ?>';
 ...
 function doValidate(form)
 {
    ...
    <?php if (!is_admin() && cpabc_get_option('dexcv_enable_captcha', CPABC_TDEAPP_DEFAULT_dexcv_enable_captcha) != 'false') { ?> if ($dexQuery("#hdcaptcha").val() == '')
    {
    ...
    { 
        <?php if (CPABC_APPOINTMENTS_ADV_DUPLICITY_VERIFICATION) { ?> var tobook = 1;
        try { 
            var tobook = document.getElementById("abc_capacity").value; 
        } catch (e) { }
        var result = $dexQuery.ajax({
           type: "GET",
           url: "<?php echo cpabc_appointment_get_site_url(); ?>?cal="+cpabc_current_calendar_item+String.fromCharCode(38)+"abc_verifydate="+$dexQuery("#selDaycal"+cpabc_current_calendar_item).val()+String.fromCharCode(38)+"tobook="+tobook,
           async: false
        }).responseText;
        if (result == "failed")
        {
...
<?php } ?>
<input type="hidden" name="form_structure" id="form_structure" size="180" value="<?php echo str_replace('"','&quot;',str_replace("\r","",str_replace("\n","",esc_attr(cpabc_appointment_translate_json(cpabc_appointment_cleanJSON(cpabc_get_option('form_structure', CPABC_APPOINTMENTS_DEFAULT_form_structure))))))); ?>" />
  <div id="fbuilder">
