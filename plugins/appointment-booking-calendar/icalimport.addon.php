...
function cpabc_iCalFetch($url)
{
    ...
    foreach ($events as $event)
        if ( strlen(@$event["DTSTART"]) > 8 && !strpos($event['UID'], "@".$_SERVER["SERVER_NAME"]) )
        {
             ...
             $processed_events[] = array(
                                          "date" => date('Y-m-d', strtotime($date ." ".($offset>0?'+':'').$offset." seconds")),
                                          "time" => date('H:i', strtotime($date ." ".($offset>0?'+':'').$offset." seconds")),
 ...
 */
function cpabc_iCalImport($calId, $icalurl)
{
	    global $wpdb;
        ...
        try
        {
            $events = cpabc_iCalFetch( $icalurl );
        } 
        catch (Exception $e) 
        {
        ...
        foreach ($events as $event)
        {
            ...
            $check = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM ".CPABC_APPOINTMENTS_TABLE_NAME." WHERE booked_time=%s AND name=%s", $date, $event["summary"] ) );              
            if (!count($check))
            {                
                $wpdb->insert( CPABC_APPOINTMENTS_TABLE_NAME, array( 'calendar' => $calId,
                                                                    'time' => current_time('mysql'),
                                                                    'booked_time' => $date,
                                                                    'booked_time_customer' => $date,
                                                                    'booked_time_unformatted' => $date,
                                                                    'name' => $event["summary"],
                                                                    'email' => "",
                                                                    'phone' => "",
                ...
                $rows_affected = $wpdb->insert( CPABC_TDEAPP_CALENDAR_DATA_TABLE, array( 'appointment_calendar_id' => $calId,
                                                                    'datatime' => $date,
                                                                    'title' => $event["summary"],
                                                                    'reminder' => '0',
                                                                    'quantity' => 1,
