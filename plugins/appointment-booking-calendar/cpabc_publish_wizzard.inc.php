...
<div class="ahb-buttons-container">
	<a href="javascript:document.location='admin.php?page=cpabc_appointments';" class="ahb-return-link">&larr;Return to the calendars list</a>
	<div class="clear"></div>
</div>

<form method="post" action="?page=cpabc_appointments&pwizard=1" name="regForm" id="regForm">          
...
<div class="ahb-section-container">
	<div class="ahb-section ahb-section-active" data-step="1">
        ...
        <div style="clear:both"></div>
        <button class="button button-primary cpmvcontainer" type="button" id="nextBtn" onclick="window.open('?page=cpabc_appointments&cal=<?php echo $_POST["cpabc_publish_id"]; ?>');">Edit the booking form settings</button>
        <div style="clear:both"></div>
    </div>
</div>
<div style="clear:both"></div>
                ...
                <tr valign="top">
					...
					<td> 
                        <select name="whereto" onchange="mvpublish_displayoption(this);">
                          ...
                          <option value="4">Widget in a sidebar, header or footer</option>
                        </select>                    
                    </td>    
                </tr> 
               ...
               <tr valign="top"  id="ppage" style="display:none">
					...
					<td>
                       ...
                       <h3 style="background:#cccccc; padding:5px;">Using classic WordPress editor or other editors?</h3>
                         ...
                         <?php _e('You can also publish the form in a post/page, use the dedicated icon','cpabc'); ?> <?php echo '<img hspace="5" src="'.plugins_url('/images/cpabc_apps.gif', __FILE__).'" alt="'.__('Insert Appointment Booking Calendar','cpabc').'" /></a>';     ?>
                         <?php _e('which has been added to your Upload/Insert Menu, just below the title of your Post/Page or under the "+" icon if using the Gutemberg editor.','cpabc'); ?>
...
function reloadappbk(item) {
    document.location = '?page=cpabc_appointments&pwizard=1&cal='+item.options[item.options.selectedIndex].value;
}
...
<div id="metabox_basic_settings" class="postbox" >
  <h3 class='hndle' style="padding:5px;"><span><?php _e('Note','cpabc'); ?></span></h3>
  <div class="inside">
   <?php _e('You can also publish the form in a post/page, use the dedicated icon','cpabc'); ?> <?php echo '<img hspace="5" src="'.plugins_url('/images/cpabc_apps.gif', __FILE__).'" alt="'.__('Insert Appointment Booking Calendar','cpabc').'" /></a>';     ?>
   <?php _e('which has been added to your Upload/Insert Menu, just below the title of your Post/Page or under the "+" icon if using the Gutemberg editor.','cpabc'); ?>
   <br /><br />
